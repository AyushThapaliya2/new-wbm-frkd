...
function normalizeHistRow(r) {
  ...
  return {
    ...
    h2s_ppm: Number.isFinite(Number(r.h2s_ppm)) ? Number(r.h2s_ppm) : null,
    nh3_ppm: Number.isFinite(Number(r.nh3_ppm)) ? Number(r.nh3_ppm) : null,
    smoke_ppm: Number.isFinite(Number(r.smoke_ppm))
      ? Number(r.smoke_ppm)
      : null,
    ch4_ppm: Number.isFinite(Number(r.ch4_ppm)) ? Number(r.ch4_ppm) : null,
  };
}
...
// H2S 0.5–50, NH3 1–300, Smoke 10–1000, CH4 1–10000.
...
export function smellRisk(d) {
  ...
  const ch4 = Number.isFinite(Number(d?.ch4_ppm)) ? Number(d.ch4_ppm) : 0;
  const h2s = Number.isFinite(Number(d?.h2s_ppm)) ? Number(d.h2s_ppm) : 0;
  const nh3 = Number.isFinite(Number(d?.nh3_ppm)) ? Number(d.nh3_ppm) : 0;
  const smoke = Number.isFinite(Number(d?.smoke_ppm)) ? Number(d.smoke_ppm) : 0;

  return (
    4 * h2s + // H2S weighted highest (safety)
    2.5 * nh3 + // NH3 next
    0.2 * smoke + // smoke broad/noisy → lower weight
    Math.min(ch4 / 50, 50) + // cap CH4 contribution
    Math.max(0, (t - 27) * 2) +
    (h / 100) * 20
  );
}
...
export function summarizeGas(rows) {
  ...
  const h2s = take("h2s_ppm");
  const nh3 = take("nh3_ppm");
  const smoke = take("smoke_ppm");
