...
// Function to update device info from website
export const updateDeviceSoftware = async (updatedDevice, deviceType) => {
  ...
  const endpoint = 'http://localhost:3000/api/update-device-software';
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      id: updatedDevice.id,
      updateData,
      tableName: deviceType === 'bin' ? 'devices' : 'weather_sensors'
    }),
  });
  ...
  if (data.error) {
    console.error('Error updating device info:', data.error);
  ...
  export const updateDeviceHardware = async (unique_id, updateFields) => {
    try {
      const response = await fetch('http://localhost:3000/api/update-device-hardware', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
