...
import { fetchWeatherDevices } from '@/lib/dataProvider';
...
export default function BinView() {
  ...
  useEffect(() => {
    if (!session) {
      ...
      router.push('/weather-sensors');
    }
  }, [session, router]);

  useEffect(() => {
    const getDevices = async () => {
      const data = await fetchWeatherDevices();
  ...
  const mapListToggle = () => {
    if (view === 'map') {
      return <MapView devices={devices} deviceType='weather' />;
    } else if (view === 'list') {
