...
export function estimateHoursUntilFull(binsData, fillRates) {
  ...
  binsData.forEach((bin) => {
    if (
      !latestData[bin.unique_id] ||
      new Date(bin.saved_time) > new Date(latestData[bin.unique_id].saved_time)
    ) {
  ...
  Object.keys(latestData).forEach((uniqueId) => {
    ...
    if (fillRate > 0) {
      // Only calculate if the fill rate is positive
...
export function binsDueForPickup(
  predictedTimes,
  thresholdInHours = THRESHOLD_IN_HOURS
) {
  const binsForPickup = [];
  ...
  Object.keys(predictedTimes).forEach((uniqueId) => {
    ...
    const thresholdTime = new Date(
      currentTime.getTime() + thresholdInHours * 3600000
